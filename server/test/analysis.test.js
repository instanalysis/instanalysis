...
jest.mock('aws-sdk', () => {
  return {
    ...
    Rekognition: class rekognition {
      constructor({ apiVersion }) {
        this.apiVersion = apiVersion
      }
      detectFaces(params, cb) {
        let data = {
          FaceDetails: [
            {
              Emotions: [
...
jest.mock('ibm-watson/personality-insights/v3', () => {
  return class personalityInsight {
    constructor({ version, iam_apikey }) {
      this.version = version
      this.aim_apikey = iam_apikey
    }

    async profile(profileParams) {
      return new Promise((resolve, reject) => {
        if(profileParams.content.contentItems[0].content === 'failed'){
          reject({code:400, message:'word count is not enough'})
        }else{
          resolve('mockdata')
        }
      })
  ...
  return function (key) {
    return {
      detectLanguage(a, cb) {
        ...
        return cb('no error', language)
      },
      translate(text, language, cb) {
        cb('no err', text)
      }
    }
  }
...
const mockData = {
  ...
  },
  {
    ...
    "caption": "Clubstyle #fxdc #clubstyle #dyna",
    "likes": 2369,
    "date": "2017"
  },
  {
...
describe('Analysis Controller Test', () => {
  describe('GET /', () => {
    ...
    test("expected response with status 200'", (done) => {
      chai
        .request(app)
        .post("/match")
        .send(mockDataCompare)
        .end(function (err, res) {
